...
Функция СформироватьТабличныйДокументАктаОСВПоУслугамПоДаннымЗаполнения(
    ...
    НомерСтроки = 0;
    Для Каждого ТитульнаяСтрокаДоговора Из ДанныеЗаполненияТЧ.Строки Цикл // Обход по договорам (содержит только договор)

        // Обход по контрагентам договора (основной цикл)
        Для Каждого СтрокаДанныхКонтрагента Из ТитульнаяСтрокаДоговора.Строки Цикл
            ...
            МассивОбластейДетальныхЗаписей = ЗаполнитьДетальныеЗаписиДокументаПоКонтрагенту(
                    СтрокаДанныхКонтрагента, ДанныеЗаполненияШапки, МакетАкта);
    ...
    СальдоНаКонецПериода = СтруктураОбщихИтогов.ОстатокНаКонецДт - СтруктураОбщихИтогов.ОстатокНаКонецКт;
    СтруктураОбщихИтогов.ОстатокНаКонецДт = Макс(СальдоНаКонецПериода, 0);
    СтруктураОбщихИтогов.ОстатокНаКонецКт = Макс(-СальдоНаКонецПериода, 0);
...
Функция ЗаполнитьДетальныеЗаписиДокументаПоКонтрагенту(
        Знач ДанныеЗаполненияТЧ, Знач ДанныеЗаполненияШапки, Знач МакетАкта = Неопределено)

    РезультатФункции = Новый Массив;

    Для Каждого СтрокаДанныхТЧ Из ДанныеЗаполненияТЧ.Строки Цикл
        Если СтрокаДанныхТЧ.Дебет = 0 И СтрокаДанныхТЧ.Кредит = 0 Тогда
            Продолжить; // Нет оборотов
        КонецЕсли;
