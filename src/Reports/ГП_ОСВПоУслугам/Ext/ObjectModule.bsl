...
Функция ИсключитьУсловиеФильтрацииДоговоровИзЗапроса(Знач ТекстЗапроса, Знач Исключить = Истина)
    Если Исключить = Истина Тогда
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Субконто2 В (&МассивДоговоров)", "И Субконто2 НЕ В (&МассивДоговоровПустой)");
    Иначе
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Субконто2 НЕ В (&МассивДоговоровПустой)", "И Субконто2 В (&МассивДоговоров)");
    КонецЕсли;

    Возврат ТекстЗапроса;
КонецФункции

Функция ИсключитьУсловиеФильтрацииКонтрагентовИзЗапроса(Знач ТекстЗапроса, Знач Исключить = Истина)
    Если Исключить = Истина Тогда
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Субконто1 В (&МассивКонтрагентов)", "И Субконто1 НЕ В (&МассивКонтрагентовПустой)");
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
                "ХозрасчетныйОбороты.КорСубконто1 В (&МассивКонтрагентов)",
                "ХозрасчетныйОбороты.КорСубконто1 НЕ В (&МассивКонтрагентовПустой)");
    Иначе
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И Субконто1 НЕ В (&МассивКонтрагентовПустой)", "И Субконто1 В (&МассивКонтрагентов)");
        ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
                "ХозрасчетныйОбороты.КорСубконто1 НЕ В (&МассивКонтрагентовПустой)",
                "ХозрасчетныйОбороты.КорСубконто1 В (&МассивКонтрагентов)");
    КонецЕсли;
...
Процедура ИнициализироватьПараметры()
    ...
    //        ЭтотОбъект.КомпоновщикНастроек, "МассивДоговоров");
    //НаборыДанных = ПолучитьОсновнойНаборДанных();
    //НаборыДанных.Запрос =
    //    ИсключитьУсловиеФильтрацииДоговоровИзЗапроса(НаборыДанных.Запрос, ПараметрМассивДоговоров.Использование = Ложь);

    //// Уточняем условие отбора по контрагентам
    //ПараметрМассивКонтрагентов = БухгалтерскиеОтчетыКлиентСервер.ПолучитьПараметр(
    //        ЭтотОбъект.КомпоновщикНастроек, "МассивКонтрагентов");
    //НаборыДанных.Запрос =
    //    ИсключитьУсловиеФильтрацииКонтрагентовИзЗапроса(НаборыДанных.Запрос, ПараметрМассивКонтрагентов.Использование = Ложь);
КонецПроцедуры
...
Функция ПодготовитьДеревоОтчета(Знач ИсходнаяТаблица)
    Если ИсходнаяТаблица.Количество() = 0 Тогда
        ИсходнаяТаблица = НовыйТаблицаОтчетаДляИтогов();
        ИсходнаяТаблица.Добавить();
    КонецЕсли;
    ...
    Запрос.Текст =
        ...
        |    ВТ_ИсходнаяТаблица.Договор.Номер КАК НомерДоговора,
...
Функция НовыйТаблицаОтчетаДляИтогов()
    РезультатФункции = Новый ТаблицаЗначений;

    РезультатФункции.Колонки.Добавить("Контрагент", Новый ОписаниеТипов("СправочникСсылка.Контрагенты"));
    РезультатФункции.Колонки.Добавить("Дата", Новый ОписаниеТипов("Дата"));
    РезультатФункции.Колонки.Добавить("Документ", Документы.ТипВсеСсылки());
    РезультатФункции.Колонки.Добавить("Договор", Новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
    РезультатФункции.Колонки.Добавить("КорСчет", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
    РезультатФункции.Колонки.Добавить("Дебет", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("Кредит", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаНачало", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаНачалоДт", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаНачалоКт", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("Оборот", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОборотДт", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОборотКт", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаКонец", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаКонецДт", Новый ОписаниеТипов("Число"));
    РезультатФункции.Колонки.Добавить("ОстатокНаКонецКт", Новый ОписаниеТипов("Число"));
