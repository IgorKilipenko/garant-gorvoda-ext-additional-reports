...
Функция ЗаполнитьПоляЧленовКомиссии(ДокументРезультат, Знач ОбластьНачалаПоиска = Неопределено)
    ...
    КонецЕсли;
    ОбластьЧленыКомиссии.Текст = СтрШаблон(
            ПолучитьШаблонПредставленияЧленаКомиссии(),
            ?(ПустаяСтрока(ДолжностьПредседателя),
                ПустаяСтрокаДолжности, ДолжностьПредседателя), ОбластьЧленыКомиссии.Текст);
    ...
    ИначеЕсли ТипЗнч(ПараметрЧленыКомиссии.Значение) <> Тип("СписокЗначений") Тогда
        ТекПредставлениеЧленаКомиссии = ПолучитьФИОФизическогоЛица(ПараметрЧленыКомиссии.Значение, ДатаАктуальностиСведений);
        ТекДолжностьЧленыКомиссии = ПолучитьПредставлениеДолжностиФизическогоЛица(
                ПараметрЧленыКомиссии.Значение, ПараметрОрганизация.Значение, ДатаАктуальностиСведений);

        ТекДолжностьЧленыКомиссии = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(
                ?(ПустаяСтрока(ТекДолжностьЧленыКомиссии),
                    ПустаяСтрокаДолжности, ТекДолжностьЧленыКомиссии), 100, " ", "Справа");

        ТекПредставлениеЧленаКомиссии = СтрШаблон(
                ПолучитьШаблонПредставленияЧленаКомиссии(),
                ?(ПустаяСтрока(ТекДолжностьЧленыКомиссии),
                    ПустаяСтрокаДолжности, ТекДолжностьЧленыКомиссии), ТекПредставлениеЧленаКомиссии);
        СписокЧленовКомиссии.Добавить(ТекПредставлениеЧленаКомиссии);

    Иначе
        Для Каждого ТекущийЧленКомиссии Из ПараметрЧленыКомиссии.Значение Цикл
            ТекПредставлениеЧленаКомиссии = ПолучитьФИОФизическогоЛица(ТекущийЧленКомиссии.Значение, ДатаАктуальностиСведений);

            ТекПредставлениеЧленаКомиссии = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(
                    ?(ПустаяСтрока(ТекПредставлениеЧленаКомиссии),
                        ПустаяСтрокаДолжности, ТекПредставлениеЧленаКомиссии), 100, " ", "Справа");

            ТекДолжностьЧленыКомиссии = ПолучитьПредставлениеДолжностиФизическогоЛица(
                    ТекущийЧленКомиссии.Значение, ПараметрОрганизация.Значение, ДатаАктуальностиСведений);
            ТекДолжностьЧленыКомиссии = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(
                    ?(ПустаяСтрока(ТекДолжностьЧленыКомиссии),
                        ПустаяСтрокаДолжности, ТекДолжностьЧленыКомиссии), 100, " ", "Справа");
            ТекПредставлениеЧленаКомиссии = СтрШаблон(
...
Функция ПолучитьПредставлениеДолжностиФизическогоЛица(Знач ФизЛицо, Знач Организация, Знач ДатаАктуальности = Неопределено)
    // Возврат Строка(УчетЗарплаты.ДолжностьФизическогоЛица(ФизЛицо, Организация, ДатаАктуальности));

    // Запрос = Новый Запрос;
    // Запрос.Текст =
    //     "ВЫБРАТЬ РАЗРЕШЕННЫЕ
    //     |   ОсновныеСотрудникиФизическихЛиц.Сотрудник КАК Сотрудник,
    //     |   ОсновныеСотрудникиФизическихЛиц.Сотрудник.Должность КАК Должность
    //     |ИЗ
    //     |   РегистрыСведений.ОсновныеСотрудникиФизическихЛиц.СрезПоследних(&ДатаАктуальности, ГоловнаяОрганизация = &Организация И ФизическоеЛицо = &ФизическоеЛицо) КАК ОсновныеСотрудникиФизическихЛиц
    //     |ГДЕ
    //     |   ОсновныеСотрудникиФизическихЛиц.ДатаНачала <= &ДатаАктуальности
    //     |   И ОсновныеСотрудникиФизическихЛиц.ДатаОкончания >= &ДатаАктуальности
    //     |";
    //
    // Если ДатаАктуальности = Неопределено Тогда
...
Функция ПолучитьШаблонПредставленияЧленаКомиссии()
    Возврат "%1" + Символы.ПС + Символы.ПС + "_________________________________ / %2";
КонецФункции
