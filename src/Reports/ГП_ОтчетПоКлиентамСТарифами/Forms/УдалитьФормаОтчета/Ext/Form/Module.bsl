...
Процедура ВыполнитьИнициализациюЭлементовОтбораНаСервере(Знач УстанавливатьЗначенияПоУмолчанию = Истина, Знач Настройки = Неопределено)
    ...
    СтруктураЭлементовДляНастройки.Вставить("ВидПотребителя",
        Новый Структура(КлючиСтруктуры, "Вид потребителя", Справочники.lc_ГруппыПотребителей.ПустаяСсылка()));
    ...
    Если УстанавливатьЗначенияПоУмолчанию Тогда
        Попытка
            Для Каждого ПараметрОтбора Из Настройки.Элементы Цикл
                ...
                ИначеЕсли ПараметрОтбора.Параметр = Новый ПараметрКомпоновкиДанных(СтруктураЭлементовДляНастройки.ВидПотребителя.Имя) Тогда
                    ПараметрОтбора.Значение = СтруктураЭлементовДляНастройки.ВидПотребителя.Значение;
                КонецЕсли;
            КонецЦикла;
        Исключение
            // Без сообщения
