...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
    ...
    ПодготовитьДанныеДляВывода(ДанныеЗаполнения, ОбщиеДанныеОтчета);
...
Процедура ПодготовитьДанныеДляВывода(Знач ДанныеЗаполнения, Знач ПараметрыЗаполнения)
    КоэффициентПересчета = ПараметрыЗаполнения.ФорматЗадолженности.Коэффициент;

    Для Каждого СтрокаДанных Из ДанныеЗаполнения Цикл
        СтрокаДанных.СуммаКонечныйОстаток = СтрокаДанных.СуммаКонечныйОстаток * КоэффициентПересчета;
        Если ПараметрыЗаполнения.ФорматЗадолженности.Значение = Перечисления.ПорядкиОкругленияОтчетности.Окр1000 Тогда
            Если СтрокаДанных.СуммаКонечныйОстаток <> 0
                И СтрокаДанных.СуммаКонечныйОстаток * СтрокаДанных.СуммаКонечныйОстаток < 1 Тогда

                СтрокаДанных.СуммаКонечныйОстаток
                = Окр(СтрокаДанных.СуммаКонечныйОстаток / СтрокаДанных.СуммаКонечныйОстаток, 0, 1);
            КонецЕсли;
        КонецЕсли;
        СтрокаДанных.СуммаКонечныйОстаток = Окр(СтрокаДанных.СуммаКонечныйОстаток, 2, 1);
        // СтрокаДанных.СуммаПросрочено = СтрокаДанных.СуммаПросрочено * КоэффициентПересчета;
    КонецЦикла;
КонецПроцедуры
