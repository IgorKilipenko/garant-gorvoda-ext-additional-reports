...
Процедура ИнициализироватьПараметры()
    ...
    ЭтоОтчетПоСчету60 = Ложь;
    ПараметрОсновнойСчетОтчета =
        БухгалтерскиеОтчетыКлиентСервер.ПолучитьПараметр(
            ЭтотОбъект.КомпоновщикНастроек, "ОсновнойСчетОтчета");
    Если ПараметрОсновнойСчетОтчета.Использование И ПараметрОсновнойСчетОтчета.Значение = "60" Тогда
        ЭтоОтчетПоСчету60 = Истина; 
    КонецЕсли;

    // Заполняем параметр ФильтрСписокСчетов
    ФильтрСписокСчетов = ПолучитьСчета(ЭтоОтчетПоСчету60);
    БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
        ЭтотОбъект.КомпоновщикНастроек, "ФильтрСписокСчетов", ФильтрСписокСчетов);

    БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
        ЭтотОбъект.КомпоновщикНастроек, "СчетаДт", ПолучитьСчетаДт(ЭтоОтчетПоСчету60));
    БухгалтерскиеОтчетыКлиентСервер.УстановитьПараметр(
        ЭтотОбъект.КомпоновщикНастроек, "СчетаКт", ПолучитьСчетаКт(ЭтоОтчетПоСчету60));
КонецПроцедуры
...
Функция ПолучитьСчета(Знач ПоСчету60 = Ложь)
    РезультатФункции = Новый СписокЗначений;

    Если ПоСчету60 = Истина Тогда
        РезультатФункции.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.02"));
        РезультатФункции.Добавить(ПланыСчетов.Хозрасчетный.НайтиПоКоду("60.01"));
    Иначе
...
// Структура:
//  СписокЗначений из ПланСчетовСсылка.Хозрасчетный
Функция ПолучитьСчетаДт(Знач ПоСчету60 = Ложь)
    РезультатФункции = Новый СписокЗначений;
...
// Структура:
//  СписокЗначений из ПланСчетовСсылка.Хозрасчетный
Функция ПолучитьСчетаКт(Знач ПоСчету60 = Ложь)
    РезультатФункции = Новый СписокЗначений;
...
Функция СформироватьТабличныйДокумент(Знач ОбщиеДанные, Знач ДанныеЗаполнения, Знач ТабличныйДокумент)
    ...
    Если ЭтоКредиторскаяЗадолженность = Истина Тогда
        ...
        ОбластьЗаголовокТаблицы.Параметры.ТипПредприятиПоЗадолженности = "кредитора";
    Иначе
        ОбластьЗаголовокТаблицы.Параметры.ВидЗадолженности = "дебиторской";
        ОбластьЗаголовокТаблицы.Параметры.ТипПредприятиПоЗадолженности = "дебитора";
    КонецЕсли;
    ...
    Для Каждого СтрокаДанныхТЧ Из ДанныеЗаполнения Цикл
        ...
        ОбластьМакета.Параметры.ИНН = СтрокаДанныхТЧ.ИНН;
