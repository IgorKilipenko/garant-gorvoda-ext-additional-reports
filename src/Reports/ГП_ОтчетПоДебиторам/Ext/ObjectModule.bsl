...
Функция СформироватьТабличныйДокумент(Знач ОбщиеДанные, Знач ДанныеЗаполнения, Знач ТабличныйДокумент)
    ...
    ОбластьСтрокаПрочее = СформироватьСтрокуПрочихЗадолженностей(
        ОбщиеДанные, ДанныеЗаполнения, Макет.ПолучитьОбласть("СтрокаТаблицыПрочие"));
    ...
    Для Каждого СтрокаДанныхТЧ Из ДанныеЗаполнения Цикл
        ...
        Иначе
            ...
            Если СуммаЗадолженностиПоСтроке <> 0 И СуммаЗадолженностиПоСтроке > ОбщиеДанные.ПорогДолга Тогда
...
Функция СформироватьСтрокуПрочихЗадолженностей(Знач ОбщиеДанные, Знач ДанныеЗаполнения, ОбластьМакета)
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ВЫБОР
        ...
        |   ВЫБОР
        |       КОГДА &ВидЗадолженности = ""Кредиторская""
        |           ТОГДА ДанныеЗаполнения.СуммаПросрочено <= &ПорогДолга
        |       ИНАЧЕ ДанныеЗаполнения.СуммаКонечныйОстаток <= &ПорогДолга
        |   КОНЕЦ
        |;
        |
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   СУММА(ВТ_ОстатокЗадолженности.СуммаЗадолженности) КАК СуммаЗадолженности
        |ИЗ
