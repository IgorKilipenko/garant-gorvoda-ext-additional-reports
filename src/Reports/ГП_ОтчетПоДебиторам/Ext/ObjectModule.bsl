...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
    ...
    ЗначениеПараметраВидЗадолженности = ПолучитьЗначениеПараметраПоНаименованию("ВидЗадолженности", "");
    ЗначениеПараметраПорогДолга = ПолучитьЗначениеПараметраПоНаименованию("ПорогДолга", 0);
...
Функция СформироватьТабличныйДокумент(Знач ОбщиеДанные, Знач ДанныеЗаполнения, Знач ТабличныйДокумент)
    ...
    Если ЭтоКредиторскаяЗадолженность = Истина Тогда
        ...
        // Поля исключены из макета по требованию Заказчика
        //
        // ОбластьЗаголовокТаблицы.Параметры.ВидЗадолженности2 = "кредитора";
    Иначе
        ОбластьЗаголовокТаблицы.Параметры.ВидЗадолженности = "дебиторской";
        
        // Поля исключены из макета по требованию Заказчика
        //
        // ОбластьЗаголовокТаблицы.Параметры.ВидЗадолженности2 = "дебитора";
    КонецЕсли;

    СтруктураИтогов = Новый Структура("Сумма, СуммаПросрочено", 0, 0);

    ОбластьСтрокаПрочее = СформироватьСтрокуПрочихЗадолженностей(
        ОбщиеДанные, ДанныеЗаполнения, Макет.ПолучитьОбласть("СтрокаТаблицыПрочие"), Макет);
    ...
    Для Каждого СтрокаДанныхТЧ Из ДанныеЗаполнения Цикл
        ...
        // Поле РасчетныйСчет исключен из макета по требованию Заказчика
        //
        ...
        КонецЕсли;
        ОбластьМакета.Параметры.Сумма = ПолучитьПредставлениеСуммы(СуммаЗадолженностиПоСтроке, ОбщиеДанные.ФорматЗадолженности);
        Если СуммаЗадолженностиПоСтроке < 0 Тогда
        ...
        Иначе
            ...
            Если СуммаЗадолженностиПоСтроке <> 0 
                И ГП_РаботаСОтчетами.Абс(СуммаЗадолженностиПоСтроке) > ГП_РаботаСОтчетами.Абс(ОбщиеДанные.ПорогДолга) Тогда
...
Функция СформироватьСтрокуПрочихЗадолженностей(Знач ОбщиеДанные, Знач ДанныеЗаполнения, ОбластьМакета, Знач Макет)
    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ВЫБОР
    ...
    Если ДанныеПоЗадолженности.Количество() = 0 ИЛИ ДанныеПоЗадолженности[0].СуммаЗадолженности = 0 Тогда
        Возврат Неопределено;
    КонецЕсли;

    СтрокаДанныхТЧ = ДанныеПоЗадолженности[0];
    ОбластьМакета.Параметры.Сумма = ПолучитьПредставлениеСуммы(СтрокаДанныхТЧ.СуммаЗадолженности, ОбщиеДанные.ФорматЗадолженности);
    Если СтрокаДанныхТЧ.СуммаЗадолженности < 0 Тогда
        ОбластьМакета.Область(1, Макет.Области.Сумма.Лево, 1,
                Макет.Области.Сумма.Право).ЦветТекста = WebЦвета.Красный;
    КонецЕсли;
