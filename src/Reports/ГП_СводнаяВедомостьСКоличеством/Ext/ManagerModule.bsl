...
Процедура ВывестиДанныеКоллекцииВДокумент(ПараметрыОтчета, Таблица = Неопределено, Дерево = Неопределено, Результат) Экспорт
	...
	НомерСтроки      = Результат.ВысотаТаблицы - (ОбластьНачисление.ВысотаТаблицы - 2);
    ПерваяКолонка    = СписокОбластейВывода.Количество() + ШиринаФиксированнойЧасти - ОбластьНачисление.ШиринаТаблицы + 1;
	
	// Учтем колонку "Всего начислено".
	Если СпУслугНачисления.Количество() > 0 Тогда
        ПерваяКолонка = ПерваяКолонка + ОбластьНачисление.ШиринаТаблицы;
	КонецЕсли;
	...
	КолонкиСуммирования = "СуммаНачисления, СуммаЛьготы, СуммаПерерасчета, СуммаПени, СуммаПовышающийКоэффициент,"
		+ " ИтоговаяСуммаНачисления, СуммаОплаты, СуммаОплатыПени, ИтогоОплачено,"
        + "КоличествоНачисления, КоличествоЛьготы, КоличествоПерерасчета, КоличествоПени, КоличествоПовышающийКоэффициент,"
        + " ИтоговоеКоличествоНачисления";
	
	ТаблицаНачислений.Свернуть("Услуга", КолонкиСуммирования);
...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат "Списки" + УПЖКХ_ТиповыеМетодыКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
...
Функция ПолучитьВнешниеНаборыДанных(ПараметрыОтчета, МакетКомпоновки) Экспорт
    ...
    //    |ПОМЕСТИТЬ ВТ_ОстаткиНачисленийКоличествоНачало
    //    |ИЗ
    //    |	РегистрНакопления.УПЖКХ_Начисления.Обороты(, &НачалоПериода, , Организация = &Организация {(ЛицевойСчет) КАК ЛицевойСчет, (Договор).* КАК Договор}) КАК УПЖКХ_НачисленияОбороты
    //    |{ГДЕ
    //    |	УПЖКХ_НачисленияОбороты.Услуга.ВидУслуги КАК УслугаДляОтбора,
    //    |	УПЖКХ_НачисленияОбороты.Договор.Владелец КАК КонтрагентНачисления}
    //    |
    //    |СГРУППИРОВАТЬ ПО
    ...
    //    |	СУММА(УПЖКХ_НачисленияОбороты.Количество) КАК Количество,
    //    |	СУММА(УПЖКХ_НачисленияОбороты.СуммаНачисленияОборот) КАК СуммаНачисленияОборот
    //    |ПОМЕСТИТЬ ВТ_ОстаткиНачисленийКоличествоКонец
    //    |ИЗ
    //    |	РегистрНакопления.УПЖКХ_Начисления.Обороты(, &КонецПериода, , Организация = &Организация {(ЛицевойСчет) КАК ЛицевойСчет, (Договор).* КАК Договор}) КАК УПЖКХ_НачисленияОбороты
    //    |{ГДЕ
    //    |	УПЖКХ_НачисленияОбороты.Услуга.ВидУслуги КАК УслугаДляОтбора,
    //    |	УПЖКХ_НачисленияОбороты.Договор.Владелец КАК КонтрагентНачисления}
    //    |
    ...
    //    |////////////////////////////////////////////////////////////////////////////////
    //    |ВЫБРАТЬ РАЗРЕШЕННЫЕ
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоКонец.ЛицевойСчет, ВТ_ОстаткиНачисленийКоличествоНачало.ЛицевойСчет) КАК ЛицевойСчет,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоКонец.Услуга, ВТ_ОстаткиНачисленийКоличествоНачало.Услуга) КАК Услуга,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоКонец.Договор, ВТ_ОстаткиНачисленийКоличествоНачало.Услуга) КАК Договор,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоНачало.СуммаНачисленияОборот, 0) КАК СуммаНаНачалоПериода,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоНачало.Количество, 0) КАК КоличествоНаНачалоПериода,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоКонец.СуммаНачисленияОборот, 0) КАК СуммаНаКонецПериода,
    //    |	ЕСТЬNULL(ВТ_ОстаткиНачисленийКоличествоКонец.Количество, 0) КАК КоличествоНаКонецПериода
    //    |ПОМЕСТИТЬ ВТ_ОстаткиНачисленийКоличествоПодготовка
    //    |ИЗ
    //    |	ВТ_ОстаткиНачисленийКоличествоКонец КАК ВТ_ОстаткиНачисленийКоличествоКонец
    //    |		ПОЛНОЕ СОЕДИНЕНИЕ ВТ_ОстаткиНачисленийКоличествоНачало КАК ВТ_ОстаткиНачисленийКоличествоНачало
    //    |		ПО ВТ_ОстаткиНачисленийКоличествоКонец.ЛицевойСчет = ВТ_ОстаткиНачисленийКоличествоНачало.ЛицевойСчет
    //    |			И ВТ_ОстаткиНачисленийКоличествоКонец.Услуга = ВТ_ОстаткиНачисленийКоличествоНачало.Услуга
    //    |			И ВТ_ОстаткиНачисленийКоличествоКонец.Договор = ВТ_ОстаткиНачисленийКоличествоНачало.Договор
    //    |;
    //    |
    //    |////////////////////////////////////////////////////////////////////////////////
    //    |ВЫБРАТЬ РАЗРЕШЕННЫЕ
    //    |	ВТ_ОстаткиНачисленийКоличествоПодготовка.ЛицевойСчет КАК ЛицевойСчет,
    //    |	ВТ_ОстаткиНачисленийКоличествоПодготовка.Услуга КАК Услуга,
    //    |	ВТ_ОстаткиНачисленийКоличествоПодготовка.Договор КАК Договор,
    //    |	СУММА(ВТ_ОстаткиНачисленийКоличествоПодготовка.СуммаНаНачалоПериода) КАК СуммаНаНачалоПериода,
    //    |	СУММА(ВТ_ОстаткиНачисленийКоличествоПодготовка.КоличествоНаНачалоПериода) КАК КоличествоНаНачалоПериода,
    //    |	СУММА(ВТ_ОстаткиНачисленийКоличествоПодготовка.СуммаНаКонецПериода) КАК СуммаНаКонецПериода,
    //    |	СУММА(ВТ_ОстаткиНачисленийКоличествоПодготовка.КоличествоНаКонецПериода) КАК КоличествоНаКонецПериода
    //    |ИЗ
    //    |	ВТ_ОстаткиНачисленийКоличествоПодготовка КАК ВТ_ОстаткиНачисленийКоличествоПодготовка
    //    |СГРУППИРОВАТЬ ПО
    //    |	Услуга,
    //    |	ЛицевойСчет,
    //    |	Договор
    ...
    //ВнешниеНаборыДанных.Вставить("ВТ_ОстаткиОбъемов", Запрос.Выполнить().Выгрузить());
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Схема.Параметры.ПовышающийКоэффициент.Значение = ПараметрыОтчета.ПовышающийКоэффициент;
    Схема.Параметры.ВидыПлощадейОбщаяПлощадь.Значение = Справочники.УПЖКХ_ВидыПлощадей.ОбщаяПлощадь;
	...
	УПЖКХ_ТиповыеМетодыКлиентСервер.УстановитьПараметр(КомпоновщикНастроек, "ПовышающийКоэффициент", ПараметрыОтчета.ПовышающийКоэффициент);
...
Процедура ВывестиСтроку(ПараметрыОтчета,
	...
	Для Каждого СтрокаУровень Из СтрокаДерева.Строки Цикл
		...
		Если НЕ ТекущийУровеньГруппировки = СтрокаУровень.Уровень() Тогда
			...
			Если ПараметрыОтчета.Перерасчет Тогда
				...
				ИмяПараметра);
			КонецЕсли;
			Если ПараметрыОтчета.ПовышающийКоэффициент Тогда
				ИмяПараметра = "Повышающий коэффициент";
				ВывестиСтрокуСПараметром(ПараметрыОтчета, Результат, Макет, СпУслугНачисления, СпУслугОплаты, ТаблицаНачислений,
					СписокОбластейВывода, СтрокаУровень, ИмяПараметра);
			КонецЕсли;
			Если ПараметрыОтчета.Пени Тогда
				ИмяПараметра = "Пени";
				ВывестиСтрокуСПараметром(ПараметрыОтчета,
	...
	Если Итог Тогда
		...
		// Если установлен флаг "Повышающий коэффициент", то выводим строку с суммами с учетом повышающих коэффициентов.
		Если ПараметрыОтчета.ПовышающийКоэффициент Тогда
			ИмяПараметра = "Повышающий коэффициент";
			ВывестиСтрокуСПараметром(ПараметрыОтчета,
									 Результат,
...
Процедура ВывестиСтрокуСПараметром(ПараметрыОтчета, Результат, Макет, СпУслугНачисления, СпУслугОплаты, ТаблицаНачислений, СписокОбластейВывода, СтрокаУровень, ИмяПараметра, Итог = Ложь)
	...
	Если СпУслугНачисления.Количество() > 0 Тогда
		...
		ИначеЕсли ИмяПараметра = "Повышающий коэффициент" Тогда
			Область.Параметры.СуммаВсего = ВрСтрокаУровень.СуммаПовышающийКоэффициент;
		КонецЕсли;	
		тмЦветФона = ПараметрыОтчета.мСоотвЦветов.Получить(СтрокаУровень.Уровень());
		Если НЕ тмЦветФона = Неопределено Тогда
			Область.Область().ЦветФона = тмЦветФона;
	...
	Если СпУслугНачисления.Количество() > 0 Тогда
		...
		Если ИмяПараметра = "Пени" Тогда
			...
			Если Итог Тогда
				Область.Параметры.СуммаОплаты = ВрСтрокаУровень.СуммаОплатыПени;
			Иначе
				Область.Параметры.Заполнить(ВрСтрокаУровень);
			КонецЕсли;
		...
		Для Каждого ТекСтрока Из СпУслуг Цикл
			...
			Иначе
				...
				ИначеЕсли ИмяПараметра = "Перерасчет" Тогда
					...
					КонецЕсли;
				ИначеЕсли ИмяПараметра = "Повышающий коэффициент" Тогда
					ИмяПоляДляЗаполненияПараметра = "СуммаПовышающийКоэффициент";
					Если ТипСуммы = "Оплаты" Тогда
						ИмяПоляДляЗаполненияПараметра = Неопределено;
					КонецЕсли;
				КонецЕсли;
			...
			Если НЕ СтрокаНачислений = Неопределено Тогда
				...
				Иначе
                    ...
                    Попытка
                    	Область.Параметры.КоличествоУслуга = СтрокаНачислений[СтрЗаменить(ИмяПоляДляЗаполненияПараметра, "Сумма", "Количество")];
                    Исключение
                        Область.Параметры.КоличествоУслуга = 0;
                    КонецПопытки;
				КонецЕсли;
			КонецЕсли;
		...
		Если ТипСуммы = "Начисления" Тогда
			Для Каждого  ТекСтрока Из СпУслуг Цикл
				...
				Для Каждого ТекСтрокаУслуги Из СтрокиУслуги Цикл
					...
					ИначеЕсли ИмяПараметра = "Повышающий коэффициент" Тогда
						СуммаПараметра = СуммаПараметра + ТекСтрокаУслуги.СуммаПовышающийКоэффициент;
					КонецЕсли;
				КонецЦикла;
...
// Получает основной макет отчета
Функция ПолучитьМакетОтчета()
    Возврат УПЖКХ_ТиповыеМетодыСервер.ПолучитьМакет("Отчет.ГП_СводнаяВедомостьСКоличеством.ПФ_MXL_Ведомость");
КонецФункции
